...
def weeks_to_feed(people, start)
    plants = [start]
    total = start
    while total < people
        fruit_produced = plants.map.with_index { |plant, index| plant*(index+1)}
        plants.unshift(fruit_produced.inject(:+))
        total = fruit_produced.sum
    end
    return (plants.length)
end

puts weeks_to_feed(8, 1)